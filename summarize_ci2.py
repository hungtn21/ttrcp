...
files = sorted(base_dir.glob("It-0-ALNS-CI2random-*reqs-RealLoc-*.txt"),
               key=lambda p: p.name)

rows = []

for fp in files:
    m = re.match(r"It-0-ALNS-CI2random-(\d+)reqs-RealLoc-(\d+)\.txt", fp.name)
    if not m:
        continue
    x = int(m.group(1))
    y = int(m.group(2))
...
lines_out = []
lines_out.append("# CheapestInsertionInit v2 + ALNS - Summary\n")
...
result = "\n".join(lines_out)
out_path = base_dir / "summary_cheapestinsertioninit_v2.md"
